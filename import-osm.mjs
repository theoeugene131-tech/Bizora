...
// Maps OSM tag values -> your data/categories.js list.
// Add more mappings any time; unmapped tags are skipped.
const CATEGORY_MAP = {
  // Fashion
  clothes: "Fashion",
  boutique: "Fashion",
...
function buildQuery(bbox) {
  ...
  const tagKeys = Object.keys(CATEGORY_MAP);
  const shopFilters = tagKeys.map((t) => `node["shop"="${t}"](${bboxStr});`).join("\n      ");
  const amenityFilters = tagKeys.map((t) => `node["amenity"="${t}"](${bboxStr});`).join("\n      ");
  const officeFilters = tagKeys.map((t) => `node["office"="${t}"](${bboxStr});`).join("\n      ");
